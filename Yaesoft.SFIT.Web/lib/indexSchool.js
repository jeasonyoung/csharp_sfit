    ...
    function loadLeftMenu(typeViewObj) {
        if (typeViewObj) {
            ...
            switch (typeViewObj.attr("id")) {
                case "index_typeView_class": //按班级。
                    typeView = "class";
                    $("#index_leftMenu h5[class=trans]").attr("title", "学校单位");
                    break;
            ...
            loadMenuData({
                ...
                fn: function(i, t) {
                    ...
                    if (typeView == "class") {
                        html += "<h6><a href=\"IndexClasses.aspx?cid=" + t.ID + "\" target=\"_blank\">" + sname + "</a></h6>";
                    } else {
                ...
                fnClick: function(obj) {
                    if (typeView == "class") {
                        cid = obj.attr("value");
                        SetSearchFilter("uid=" + uid + "&cid=" + cid);
                    } else if (typeView == "catalog") {
                        sid = obj.attr("value");
                        SetSearchFilter("uid=" + uid + "&sid=" + sid);
                    } else if (typeView == "time") {
                        st = obj.attr("value");
                        SetSearchFilter("uid=" + uid + "&st=" + st);
                    }
                    $("#newWorkTitle").trigger("click");
                }
            });
